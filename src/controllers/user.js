...
var jwt = require('../services/jwt-simple');
var fs = require('fs');
    ...
    if(email && password){
        User.findOne({email: email.toLowerCase()}, (err, user) => {
            if(err){
                ...
                if(!user){
                    res.status(404).send({message: 'El correo electrónico que has introducido no coincide con ninguna cuenta. Regístrate para crear una cuenta.'});
                }else{
                    //Check password
